...
export default function ExchangeRates() {
  ...
  return (
    <Box className="flex justify-center bg-gradient-to-r from-[rgb(20,20,105)] to-[rgb(15,15,65)] w-full h-[1000px]">
          ...
          <Box className="flex flex-col items-center py-10 px-5 rounded-3xl bg-white w-full md:w-5/6 lg:w-4/5">
            ...
            <TextField
              ...
              onChange={(e: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
                const fixedValue = e.target.value.replace(/[۰-۹]/g, d => String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d)))
                const num = parseFloat(fixedValue);
                if (!isNaN(num)) setAmount(Number(num.toFixed(2)));
                else setAmount(0);
              }}
              value={amount}
